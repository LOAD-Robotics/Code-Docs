...
{
	// All code below this point is inserted inside these brackets
}
```
##### Definition of hardware variables
```java 
// Drive motors   
...
```java
// Variable to store the time passed since the start of the opmode
   private ElapsedTime runtime = new ElapsedTime(); 
// Variable to store battery data    
double MinBatteryVoltage = 9999999;  
// Variables to store data for the drivetrain  
double Y  = 0;  
double X  = 0;  
double rX = 0;  
double d = 0;  
...
/* Variable for storing zero offset for the hanging arm
* Positive offset moves the arm down towards the zero position, Negative moves it away */ 
int zeroOffset_Hanging = 15;  
// SAFETY MODE (Disables hanging arm and winches, and locks drivetrain speed to ~30%)
...
@Override
public void init() {
	// Code to run ONCE when the driver hits INIT is put inside this function
}
```
##### Hardware mapping
```java
...
```
##### Hardware behaviour definition
```java
/* Define the behavior of the drive motors. To drive forward, most robots need the motor on one side to be reversed,
* because the axles point in opposite directions. Pushing the left stick forward MUST make robot go forward.
* So adjust these two lines based on your first test drive. Note: The settings here assume direct drive on left and right wheels.
...
/* Set the kind of pods used by your robot. If you're using goBILDA odometry pods,
* select either the goBILDA_SWINGARM_POD, or the goBILDA_4_BAR_POD.
* If you're using another kind of odometry pod, fill in the function with the number of ticks per mm of your odometry pod. */        odo.setEncoderResolution(GoBildaPinpointDriver.GoBildaOdometryPods.goBILDA_4_BAR_POD);  
...
* and the Y (strafe) pod should increase when you move the robot to the left. */
odo.setEncoderDirections(GoBildaPinpointDriver.EncoderDirection.FORWARD, GoBildaPinpointDriver.EncoderDirection.FORWARD);  
/* Before running the robot, recalibrate the IMU. This needs to happen when the robot is stationary.
* The IMU will automatically calibrate when first powered on,
* but recalibrating before running the robot is a good idea to ensure that the calibration is "good". 
* resetPosAndIMU will reset the position to 0,0,0 and also recalibrate the IMU.
* This is recommended before you run your autonomous, as a bad initial calibration can cause an incorrect starting value for x, y, and heading. */
//odo.recalibrateIMU();
odo.resetPosAndIMU();  
```
#### Function `void init_loop()`
##### Function declaration
```java
@Override
public void init_loop() {
	// Code to be run REPEATEDLY during the period after the driver hits INIT, but before they hit START, is put in this function.
}  
```
#### Function `void start()`
##### Function declaration
```java
public void start() {
	// Code to be run ONCE what the driver hits START is put in this function.
	runtime.reset(); // TODO: Look up what this command does and add a comment about it.
}  
```
#### Function `void loop()`
##### Function declaration
...
@Override
public void loop() {
	// Code to be run REPEATEDLY after the driver hits START is put in this function.
}  
```


# Remaining Code to be annotated
```java  
